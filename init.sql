ALTER SESSION SET CURRENT_SCHEMA = DEN_DAVI;

DROP TABLE DEN_DAVI.PROTOCOL;
DROP TABLE DEN_DAVI.CRITERION;
DROP TABLE DEN_DAVI.READING;
DROP TABLE DEN_DAVI.ROAD;
DROP TABLE DEN_DAVI.CAMERA;
DROP TABLE DEN_DAVI."person";
...
CREATE UNIQUE INDEX ux_user_email ON "person" (email);

CREATE TABLE road (
  road_id     NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
  address     VARCHAR2(200) NOT NULL,
  road_speed  NUMBER(5,2) NOT NULL,
  created_at  TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP,
  updated_at  TIMESTAMP WITH TIME ZONE
);

CREATE TABLE criterion (
  criterion_id  NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
  name          VARCHAR2(100) NOT NULL,
  created_at    TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP,
  created_by    NUMBER REFERENCES "person"(user_id)
);
CREATE UNIQUE INDEX ux_criterion_name ON criterion (name);
...
CREATE UNIQUE INDEX ux_protocol_name ON protocol (name);
...
CREATE TABLE criterion_level (
  ...
  cl_level           NUMBER(5) NOT NULL, -- "level" is an invalid column name
  created_at         TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP,
  created_by         NUMBER REFERENCES "person"(user_id)
);
...
CREATE TABLE alert (
  ...
  criterion_id NUMBER NOT NULL REFERENCES criterion(criterion_id),
  protocol_id NUMBER REFERENCES protocol(protocol_id),
  ale_level       NUMBER(5), -- "level" is an invalid column name
  ale_status      VARCHAR2(20), -- "status" is an invalid column name
...
CREATE INDEX ix_alert_protocol_id   ON alert(protocol_id);
...
CREATE TABLE alert_log (
  ...
  al_status      VARCHAR2(20) -- "status" is an invalid column name
);
